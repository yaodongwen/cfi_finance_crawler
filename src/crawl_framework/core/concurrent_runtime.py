    ...
    async def run_dataset(
        ...
        async def discover() -> None:
            try:
                ...
                for _ in range(self.crawl_workers):
        ...
        try:
            await asyncio.gather(tasks[0], *tasks[1:1 + self.crawl_workers])
            for _ in range(self.writer_workers):
                await record_queue.put(sentinel)
            await asyncio.gather(
                *tasks[1 + self.crawl_workers:1 + self.crawl_workers + self.writer_workers]
            )
            for _ in range(self.upload_workers):
                await upload_queue.put(sentinel)
            await asyncio.gather(
                *tasks[
                    1 + self.crawl_workers + self.writer_workers:
                    1 + self.crawl_workers + self.writer_workers + self.upload_workers
                ]
            )
            for _ in range(self.catalog_workers):
